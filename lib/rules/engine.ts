...
function checkVictory(next: GameState, rules: RulesConfig): void {
  ...
  if (cultRule !== null) {
    const cultAlive = aliveOf(next, 'cult')
    const converts = cultAlive.filter(isConvert).length
    ...
    // 전향형 조건만 두면 **소프트락**이 생긴다 (§3 이 경고한 형태). `minConverts` 를 2 이상으로
    // 올리면 "생존자가 전부 교주팀인데 사제 수가 문턱에 못 미치는" 상태가 만들어지고,
    // 그때는 어느 진영도 이길 수 없어 밤낮이 영원히 반복된다(측정: 48 일차 교착).
    // 남이 아무도 없으면 교주팀도 전멸형으로 이긴다.
    if (cultAlive.length > 0 && outsiders === 0) {
      win(next, 'cult')
      return
    }
  }
 ...
 * 이 어긋남이 교주팀에게 정보 우위를 준다 — 3파전에서 교주팀이 정보로 이득을 보는
 * 유일한 경로여서 의도적으로 남겼다.
 *
 * 죽은 동료도 포함한다(알던 사실이 죽음으로 지워지지는 않는다). 살아있는 사람만 필요하면
 * 호출자가 걸러 쓴다.
 */
export function knownAllies(state: GameState, rules: RulesConfig, characterId: string): Character[] {
  ...
  const knows =
    origin === 'mafia'
      ? rules.knowledge.mafia
      : origin === 'citizen'
        ? rules.knowledge.citizen
 ...
 */
/**
 * 이 사람이 그 호출에서 **쓸 수 있는 능력이 실제로 있었는가.**
 *
 * 시간초과 페널티는 "쓸 수 있었는데 쓰지 않은" 사람에게만 정당하다 (§6.1). 호출 목록은
 * 역할로 뽑히므로, 역할은 그대로인데 능력이 사라진 사람이 섞인다 — 대표적으로 **포교당한
 * 마피아**다(§5.4 사제는 원래 역할을 유지한다). 그 사람은 마피아팀 살해를 할 수 없는데도
 * 마피아 호출에 불려 나와 "행동 없음"으로 죽었다(측정: 600판 348회).
 */
function couldAct(state: GameState, rules: RulesConfig, character: Character): boolean {
  switch (character.roleId) {
    case 'mafia':
    case 'bomber':
      // 팀 살해는 마피아팀의 행동이다. 전향하면 권한이 사라진다.
      return character.faction === 'mafia' && state.characters.some((c) => c.alive && c.faction !== 'mafia')
    case 'cultleader':
      return state.characters.some(
        (c) =>
          ...
          c.id !== character.id &&
          c.faction !== 'cult' &&
          (rules.cult.canConvertMafia || c.faction !== 'mafia'),
      )
    case 'police':
      return state.characters.some((c) => c.alive && (rules.police.allowSelf || c.id !== character.id))
    case 'doctor':
      return state.characters.some((c) => c.alive && (rules.doctor.allowSelf || c.id !== character.id))
    default:
      return true
  }
}
...
function applyTimeoutDeaths(
  ...
  for (const step of steps) {
    ...
    for (const id of step.actorIds) {
      ...
      // 판정은 밤이 열릴 때의 상태로 한다 — 그 밤에 벌어진 일로 소급 처벌하지 않는다.
      const asOfNightStart = atNightStart.characters.find((c) => c.id === id)
      if (asOfNightStart && !couldAct(atNightStart, rules, asOfNightStart)) continue
 ...
 *
 * "같은 팀"의 기준은 **그 밤이 열릴 때의 마피아팀**(`wasMafia`)이다. 포교는 살해보다 먼저
 * 해소되므로(§6), 지금 진영을 보면 그 밤에 전향된 사람 때문에 후보 자격과 후보 정원이
 * 뒤바뀐다 — 후보를 고른 시점에는 합법이었는데 해소 시점에 불법이 되는 것이다.
 */
function validateBombCandidates(
  ...
  for (const id of candidates) {
    ...
    if (!rules.bomber.canTargetOwnFaction && wasMafia.has(candidate.id)) {
  ...
  const eligible = next.characters.filter(
    (c) => c.alive && (rules.bomber.canTargetOwnFaction || !wasMafia.has(c.id)),
  ).length
 ...
 *
 * 진영 판정은 **밤이 열릴 때의 마피아팀**(`wasMafia`) 기준이다. 사회자 호출 순서는
 * 마피아가 먼저이고 교주가 마지막이므로(§6.1), 마피아는 자기가 전향되기 **전에** 이미
 * 지목을 마쳤다. 해소 순서(포교 → 살해)만 보고 현재 진영으로 판정하면, 그 밤에 포교당한
 * 마피아의 이미 끝난 지목이 소급해서 무효가 된다.
 */
function resolveKill(
  ...
  if (!wasMafia.has(actor.id)) throw new RuleError(`밤 살해는 마피아팀의 행동이다: ${action.actorId}`)

  const target = find(next, action.targetId)
  if (!target.alive) throw new RuleError(`이미 죽은 캐릭터를 살해할 수 없다: ${action.targetId}`)
  if (wasMafia.has(target.id) && !(actor.roleId === 'bomber' && rules.bomber.canTargetOwnFaction)) {
    throw new RuleError(`마피아는 같은 팀을 살해하지 않는다: ${action.targetId}`)
  }

  if (action.candidates !== undefined && actor.roleId !== 'bomber') {
    throw new RuleError('후보 명단은 폭탄마만 낸다 — 일반 마피아는 한 명을 지목한다')
  }

  // 폭탄마가 후보 명단 없이 지목하면 **평범한 살해**다.
  //
  // 폭탄에 횟수 제한이 있으면(Q12b) 다 쓴 폭탄마는 밤에 아무것도 할 수 없게 되고, 마피아 호출은
  // 필수라서(§6.1) 그 자리에서 시간초과로 죽었다(측정: 600판 348회). 폭탄마도 마피아팀이고
  // 팀 살해 1회는 그대로 남아 있으므로, 폭탄이 없으면 평범하게 죽인다.
  const useBomb = actor.roleId === 'bomber' && action.candidates !== undefined
  if (!useBomb) {
  ...
  const candidates = validateBombCandidates(next, rules, actor, action, wasMafia)
...
export function resolveNight(state: GameState, rules: RulesConfig, actions: NightActions): GameState {
  ...
  // 이 밤이 열릴 때의 마피아팀. 포교로 진영이 바뀌어도 이 밤의 살해 판정 기준은 여기다.
  const wasMafia = new Set(state.characters.filter((c) => c.faction === 'mafia').map((c) => c.id))
  ...
  if (actions.kill !== null) resolveKill(next, rules, actions.kill, protectedIds, wasMafia)

  applyTimeoutDeaths(next, steps, acted, rules, state)
