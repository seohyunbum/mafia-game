...
import { resolveDay, resolveTrial, verdictVoters } from '../src/core/engine.ts'
...
test('동표면 아무도 법정에 세우지 않고 그 날은 넘어간다 (🟡 Q15)', () => {
  const after = resolveDay(dayState(), RULES, { c1: 'm1', m1: 'c1' }, rng)

  assert.equal(after.nominee, null)
  assert.equal(after.phase, 'dusk', 'Trial 을 건너뛴다')
  assert.deepEqual(kinds(after), ['no_nomination'])
})
...
test('정확히 반이면 살린다 — 동표는 무죄 (🟡 Q15)', () => {
  // 자격자 4명 중 2명이 죽인다 → 과반 미달
  const state = stateOf(
    [
      character('m1', 'mafia'),
      character('c1', 'citizen'),
  ...
  assert.equal(isAlive(after, 'm1'), true)
...
test('변신한 마피아를 죽이면 피해는 흉내낸 원본 시민이 받는다 (Q1 확정)', () => {
  ...
  assert.equal(isAlive(after, 'm1'), true, '마피아 본인은 죽지 않는다')
  assert.equal(hpOf(after, 'm1'), 2, '마피아 본인은 무피해다')
  assert.equal(hpOf(after, 'c1'), 1, '흉내낸 원본 시민이 대신 1 깎인다')
...
test('같은 얼굴에 두 번 속으면 그 시민이 죽는다 — HP 2 가 쓰이는 유일한 경로', () => {
  const state = trialState('m1')
  const mafia = state.characters.find((c) => c.id === 'm1')!
  const victim = state.characters.find((c) => c.id === 'c1')!
  mafia.disguisedAs = 'c1'
  victim.hp = 1 // 어제 이미 한 번 속았다
  ...
  assert.equal(isAlive(after, 'm1'), true, '마피아는 여전히 살아 있다')
})

test('변신 흡수로 마지막 시민이 죽으면 마피아팀 승리다', () => {
  const state = stateOf(
